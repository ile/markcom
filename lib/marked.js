...
var inline = {
  // escape: /^\\([\\`*{}\[\]()#+\-.!_>])/,
  escape: /^\\([\\`*{}\[\]()#+\-.!_>\$])/, /* https://github.com/adam-p/markdown-here/ */
  autolink: /^<([^ >]+(@|:\/)[^ >]+)>/,
  url: noop,
  tag: /^<!--[\s\S]*?-->|^<\/?\w+(?:"[^"]*"|'[^']*'|[^'">])*?>/,
  link: /^!?\[(inside)\]\(href\)/,
...
inline.pedantic = merge({}, inline.normal, {
  math: noop, /* https://github.com/adam-p/markdown-here/  */
  ...
  while (src) {
    ...
    if (cap = this.rules.autolink.exec(src)) {
      ...
      out += '<a href="'
        + href
        + '"'
        + (this.options.linksToNewTab? ' target="_blank"': '')
        + '>'
        + text
        + '</a>';
      continue;
    ...
    if (cap = this.rules.text.exec(src)) {
      src = src.substring(cap[0].length);
      out += this.options.inAttribute
        ? this.smartypants(cap[0])
